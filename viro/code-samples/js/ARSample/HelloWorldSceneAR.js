...
var HelloWorldSceneAR = createReactClass({
    ...
    return (
      <ViroARScene onTrackingUpdated={this._onTrackingUpdated}>
        ...
        <ViroBox position={[0, -.5, -1]}
          animation={{name: this.state.currentAnim, run: true, loop: true, interruptible: true}}
            scale={[.3, .3, .1]} materials={["grid"]} onClick={this._switchAnimation} />
  ...
  _switchAnimation() {
    if (this.state.currentAnim == "rotate") {
        this.setState({
            currentAnim: "non",
        });
    } else {
